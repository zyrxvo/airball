...
def hyperbolic_elements(sim, star, rmax, values_only=False):
    ...
    a = -star.b / _np.sqrt(e**2.0 - 1.0)  # Compute the semi-major axis of the flyby star
...
def isList(val) -> bool:
    """Determine if an object is a list or numpy array."""
    if isinstance(val, (list, _np.ndarray)):
        return not (isinstance(val, _u.Quantity) and _np.shape(val) == ())
    return False
