...
def add_star_to_sim(sim, star, rmax=1e5, plane=None):
    ...
    e = tools.determine_eccentricity(sim, m, b, star_v=v, star_e=None) # TODO: Allow user to define e instead of v.
    ...
    rmax = tools.verify_unit(rmax, u.au)
    ...
    if plane is not None:
        ...
        elif plane == 'ecliptic': rot = _rebound.Rotation.to_new_axes(newz=tools.calculate_angular_momentum(sim)[3]) # Assumes Earth is particle 3. 0-Sun, 1-Mecury, 2-Venus, 3-Earth, ...
    elif isinstance(plane, int_types): rot = _rebound.Rotation.to_new_axes(newz=tools.calculate_angular_momentum(sim)[int(plane)])
    sim.rotate(rot)
...
def hybrid_flyby(sim, star, rmax=1e5, crossOverFactor=30, overwrite=False, integrator='whckl', heartbeat=None, particle_index=1, plane=None):
    '''
...
def hybrid_flybys(sims, stars, **kwargs):
    '''
        ...
        integrator : String.
            The integrator to use for the main integration (IAS15 is used for the close encounter). Default is WHCKL.
...
def flyby(sim, star, rmax=1e5, overwrite=False, heartbeat=None, plane=None):
    '''
        Simulate a stellar flyby to a REBOUND simulation.
