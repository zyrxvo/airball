...
EXAMPLES = [path for path in ROOT_DIR.glob("*.ipynb")]
print(EXAMPLES)
...
def test_notebooks(convert_ipynb_to_py):
    ...
    output = subprocess.run([str(sys.executable), str(convert_ipynb_to_py)], shell=False, capture_output=True, check=False, text=True)
