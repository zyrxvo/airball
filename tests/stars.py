...
def test_initialize_Stars_with_lists_and_units():
    ...
    stars = airball.Stars(m=[1*u.solMass, 2000*u.jupiterMass, 1e6*u.earthMass], b=[4*u.au, 5*u.au, 6*u.au], v=[7*u.km/u.s, 8*u.km/u.s, 9*u.km/u.s])
...
def test_initialize_Stars_with_ndarray_shapes():
    stars = airball.Stars(m=np.array([[1,2,3],[1,2,3]])*u.solMass, b=np.array([[4,5,6], [4,5,6]])*u.au, v=np.array([[7,8,9], [7,8,9]])*u.km/u.s)
    assert np.all(stars.m == np.array([[1,2,3],[1,2,3]]) * u.solMass)
    assert np.all(stars.b == np.array([[4,5,6], [4,5,6]]) * u.au)
    assert np.all(stars.v == np.array([[7,8,9], [7,8,9]]) * u.km/u.s)
...
def test_initialize_Stars_with_ndarray_odd_shapes():
    with pytest.raises(airball.stars.ListLengthException):
        airball.Stars(m=np.array([[1,2,3],[1,2,3]])*u.solMass, b=np.array([[4,5], [4,6]])*u.au, v=np.array([7,8,9])*u.km/u.s)
    with pytest.raises(airball.stars.ListLengthException):
        airball.Stars(m=np.array([[1,2,3],[1,2,3],[1,2,3]])*u.solMass, b=np.array([[4,5], [4,6]])*u.au, v=np.array([7,8])*u.km/u.s)
    with pytest.raises(airball.stars.ListLengthException):
        airball.Stars(m=np.array([[1,2,3],[1,2,3]])*u.solMass, b=np.array([[4,5], [4,6]])*u.au, v=np.array([7,8])*u.km/u.s)
...
def test_access_Stars_using_ints():
    stars = airball.Stars(m=[1,2,3], b=[4,5,6], v=[7,8,9], inc=[3,2,1], omega=[6,5,4], Omega=[9,8,7])
    assert stars[0] == airball.Star(m=1, b=4, v=7, inc=3, omega=6, Omega=9)

def test_access_multidimensional_Stars_using_ints():
    stars = airball.Stars(m=[[1,2,3],[2,2,3]], b=[[4,5,6],[5,5,6]], v=[[7,8,9],[8,8,9]], inc=[[3,2,1], [4,2,1]], omega=[[6,5,4],[7,5,4]], Omega=[[9,8,7],[10,8,7]])
    assert stars[0] == airball.Stars(m=[1,2,3], b=[4,5,6], v=[7,8,9], inc=[3,2,1], omega=[6,5,4], Omega=[9,8,7])
...
def test_Stars_equality():
    stars1 = airball.Stars(m=[1,2,3], b=[4,5,6], v=[7,8,9], inc=[3,2,1], omega=[6,5,4], Omega=[9,8,7])
    stars2 = airball.Stars(m=[1,2,3], b=[4,5,6], v=[7,8,9], inc=[3,2,1], omega=[6,5,4], Omega=[9,8,7])
