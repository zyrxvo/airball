...
class StellarEnvironment:
    ...
    def random_stars(self, size=1, **kwargs):
        ...
        seed = np.random.randint(0, (2**32 - 6)) if self.seed is None else self.seed
    ...
    def stats(self):
        ...
        star_or_object = "Stellar" if self.object_unit.to_string() == u.stars.to_string() else "Object"
        s = self.name
        s += "\n------------------------------------------\n"
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, StellarEnvironment):
            ...
            for attr in attrs:
                ...
                if not equal_attribute and isQuantity(getattr(self, attr)):
