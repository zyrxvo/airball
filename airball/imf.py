...
"""Initial Mass Functions (IMFs) for `airball`."""
...
class chabrier_2003_single(Distribution):
    r"""Chabrier 2003 IMF for single stars.
...
class chabrier_2005_single(Distribution):
    r"""Chabrier 2005 IMF for single stars.
...
class kroupa_1993(Distribution):
    r"""Kroupa et al. (1993) IMF for single stars.
...
class IMF:
    ...
    def __init__(
        ...
        if self._max_mass <= self._min_mass:
            raise ValueError("Cannot have maximum mass value be less than or equal to minimum mass.")
        ...
        if mass_function is None:
            ...
            raise ValueError("mass_function must be a function or a Distribution object.")
    ...
    def _recalculate(self):
        """Recalculate the IMF properties based on the current parameters.
        ...
        self.normalization_factor = _quad(self._pdf_, self.min_mass.value, self.max_mass.value)[0]
        # Generate logarithmically spaced mass values between min_mass and max_mass
        self._masses = _np.geomspace(self.min_mass, self.max_mass, self.number_samples).value
    ...
    def random_mass(self, size=1, **kwargs):
        """Random mass values from the IMF.
    ...
    def masses(self, number_samples, endpoint=True, unitless=True):
        """Array of mass values logarithmically spanning the IMF range.
        ...
        """
        if unitless:
            return _np.geomspace(self.min_mass, self.max_mass, int(number_samples), endpoint=endpoint).value
        return _np.geomspace(self.min_mass, self.max_mass, int(number_samples), endpoint=endpoint)
    ...
    def min_mass(self, value):
        ...
        if value.value <= 0:
            message = "Cannot have minimum mass value be less than or equal to 0."
    ...
    def number_samples(self):
        """Number of samples to use for interpolating the CDF.
